...
namespace GraphSharp.Algorithms.Layout
{
	...
	public static class LayoutUtil
	{
        ...
        //public static SKPoint GetClippingPoint(SKSize size, SKPoint s, SKPoint t)
        //{
        //    float[] sides = new float[4];
        //    sides[0] = (s.X - size.Width / 2.0f - t.X) / (s.X - t.X);
        //    sides[1] = (s.Y - size.Height / 2.0f - t.Y) / (s.Y - t.Y);
        //    sides[2] = (s.X + size.Width / 2.0f - t.X) / (s.X - t.X);
        //    sides[3] = (s.Y + size.Height / 2.0f - t.Y) / (s.Y - t.Y);
