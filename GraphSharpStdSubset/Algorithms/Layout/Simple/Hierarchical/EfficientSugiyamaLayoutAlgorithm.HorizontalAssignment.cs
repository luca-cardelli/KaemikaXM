...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        private void DoOrthogonalEdgeRouting()
        {
            ...
            {
                SKPoint[] orthoRoutePoints = new SKPoint[2];
                ...
                orthoRoutePoints[sourceIndex] = new SKPoint()
                {
                ...
                orthoRoutePoints[targetIndex] = new SKPoint()
                {
            ...
            {
                SKPoint[] orthoRoutePoints = _edgeRoutingPoints[kvp.Key];

                var routePoints = new SKPoint[kvp.Value.Count + 4];
                routePoints[0] = orthoRoutePoints[0];
                routePoints[kvp.Value.Count + 3] = orthoRoutePoints[1];
                for (int i = 0; i < kvp.Value.Count; i++)
                {
                    var vertex = kvp.Value[i];
                    routePoints[i+2] = new SKPoint(vertex.HorizontalPosition, vertex.VerticalPosition);
                }
                routePoints[1] = new SKPoint(routePoints[2].X, routePoints[0].Y);
                routePoints[kvp.Value.Count + 2] = new SKPoint(routePoints[kvp.Value.Count + 1].X, routePoints[kvp.Value.Count + 3].Y);
        ...
        private void DoTraditionalEdgeRouting()
        {
            foreach (var kvp in _dummyVerticesOfEdges)
            {
                var routePoints = new SKPoint[kvp.Value.Count];
                for (int i = 0; i < kvp.Value.Count; i++)
                {
                    var vertex = kvp.Value[i];
                    routePoints[i] = new SKPoint(vertex.HorizontalPosition, vertex.VerticalPosition);
                }
                _edgeRoutingPoints[kvp.Key] = routePoints;
            }
        ...
        private void SavePositions()
        {
            foreach (var vertex in _graph.Vertices)
            {
                if (vertex.Type == VertexTypes.Original)
                    VertexPositions[vertex.OriginalVertex] = new SKPoint(vertex.HorizontalPosition, vertex.VerticalPosition);
            }
        ...
        {
            foreach (var vertex in _graph.Vertices)
                vertex.VerticalPosition = _layerPositions[vertex.LayerIndex] + (vertex.Size.Height <= 0 ? _layerHeights[vertex.LayerIndex] : vertex.Size.Height) / 2.0f;
        }
                    ...
                    foreach (var edge in edges)
                    {
                        ...
                        if (v.Sinks[modeIndex] != u.Sinks[modeIndex])
                        {
                            ...
                            u.Sinks[modeIndex].Shifts[modeIndex] = leftRightMode == LeftRightMode.Left
                                ? (float)Math.Min(u.Sinks[modeIndex].Shifts[modeIndex], s)
                                : (float)Math.Max(u.Sinks[modeIndex].Shifts[modeIndex], s);
                        }
                        else
                        {
                            v.HorizontalPositions[modeIndex] =
                                leftRightMode == LeftRightMode.Left
                                    ? (float)Math.Max(v.HorizontalPositions[modeIndex], u.HorizontalPositions[modeIndex] + xDelta)
                                    : (float)Math.Min(v.HorizontalPositions[modeIndex], u.HorizontalPositions[modeIndex] - xDelta);
                        }
                    }
                }
                if (wSegment != null)
