...
{
    public class GUI_Windows2 : GuiInterface {
        private GUI2 form;
        public GUI_Windows2(GUI2 form) {
            this.form = form;
        }
        ...
        public override string Platform() { return "Windows"; }
        ...
        public override void DeviceUpdate() {
            if (GUI2.deviceControl == null) return;
            if (!GUI2.deviceControl.Visible) return;
            if (GUI2.deviceControl.InvokeRequired) {
                VoidArgVoidReturnDelegate d = new VoidArgVoidReturnDelegate(DeviceUpdate);
                GUI2.deviceControl.Invoke(d, new object[] { });
            } else {
                GUI2.deviceControl.Size = form.panel_Microfluidics.Size;
                GUI2.deviceControl.Invalidate();
                GUI2.deviceControl.Update();
            }
        }
        ...
        public override Noise NoiseSeries() {
            if (form.button_Noise.InvokeRequired) {
        ...
        public override string Solver() {
            if (form.panel_Settings.InvokeRequired) {
