...
namespace Kaemika {
    ...
    public interface DevicePainter : Painter { // interface for platform-dependent device rendering
        void Draw(ProtocolDevice.Device device, int canvasX, int canvasY, int canvasWidth, int canvasHeight);
    }

    // "The" current Device is stored here, is viewed inside the GUI by the XXDeviceView of each platform, and is painted by the XXDevicePainter
    // The platform-independent GUIinterface accesses the platform-dependent GUI and through it the XXDeviceView
