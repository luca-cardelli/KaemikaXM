    ...
    public static class Export {
        ...
        public static void Snap(Func<Colorer> GenColorer, Func<SKSize, ChartPainter> GenPainter, SKSize chartSize) {
            try {
                ...
                chartPainter.Clear(SKColors.White);
                KChartHandler.DrawOver(chartPainter, 0, 0, (int)chartSize.Width, (int)chartSize.Height);
                KChartHandler.DrawLegend(chartPainter, new SKPoint(chartSize.Width, header), legendSize, pointSize);
                ...
                KScoreHandler.DrawOver(chartPainter, 0, (int)(maxChartLegReactHeight + KScore.Yspacing), (int)scoreSize.Width, (int)scoreSize.Height);
            } catch { }
        }
        ...
        //        string reactants = CRN.FormatCanonical(r.reactants, style);
        //        if (!names.ContainsKey(reactants)) names[reactants] = new Vertex_Label(reactants);
        //        graph.AddVertex(names[reactants]);
        //        string products = CRN.FormatCanonical(r.products, style);
        ...
        //    foreach (ReactionValue r in reactions) {
        //        GraphAddEdge(graph, names[CRN.FormatCanonical(r.reactants, style)], names[CRN.FormatCanonical(r.products, style)]);
        //    }
        //}

        //public static Dictionary<Symbol, int> NormalizedComplex(List<Symbol> complex, Style style) {
        //    Dictionary<Symbol, int> l = new Dictionary<Symbol, int>();
        //    foreach (Symbol symbol in complex) {
        //        if (!l.ContainsKey(symbol)) l[symbol] = 0;
        //        l[symbol]++;
        //    }
        //    return l;
        //}
        ...
        //            int n = catalystsMset.Cardinality(catalyst);
        ...
        //            int n = reactantsMset.Cardinality(reactant);
        //            GraphAddEdge(graph, names[reactant.Format(style)], reaction, null, Directed.Pointy);
        //            for (int i = 1; i < n; i++) { // all edges except the first
        //                GraphAddRoutedEdge(graph, names[reactant.Format(style)], reaction, null, Directed.Pointy);
        //            }
        ...
        //            int n = productsMset.Cardinality(product);
        //            GraphAddEdge(graph, reaction, names[product.Format(style)], null, Directed.Solid);
        //            for (int i = 1; i < n; i++) { // all edges except the first
        //                GraphAddRoutedEdge(graph, reaction, names[product.Format(style)], null, Directed.Solid);
        //            }
