...
namespace Kaemika {
    ...
    public class Type {
        ...
        public bool Matches(Value value) {
            return
                ...
                ((this.type == "omega") && (value is OmegaValue)) ||
                ((this.type == "random") && (value is DistributionValue)) ||
    ...
    public abstract class Value {
        ...
        public Type type;
        public bool EqualValue(Value other, Style style) {
            if (this is BoolValue && other is BoolValue) return (this as BoolValue).value == (other as BoolValue).value;
            ...
            else if (this is ListValue<Flow> && other is ListValue<Flow>) return (this as ListValue<Flow>).EqualList(other as ListValue<Flow>, style);
            else return false; // we do not give error on 3=true because anyway [3,4]=[true] would return false just because the lengths are different
        }
    ...
    public class ListValue<T> : Value where T:Value {
        ...
        public bool EqualList(ListValue<T> other, Style style) {
            ...
            for (int i = 0; i < elements.Count; i++) {
                if (!elements[i].EqualValue(otherElements[i], style)) return false;
            }
            return true;
        }
    ...
    public class OperatorValue : Value {
        ...
        public Value Apply(List<Value> arguments, Netlist netlist, Style style, int s) {
        ...
        public Value ApplyFlow(List<Value> arguments, Style style) { // a subset of Apply
            ...
            if (arguments.Count == 0) {
                ...
                else if (name == "++") if (arg1 is ListValue<Value> && arg2 is ListValue<Value>) return (((ListValue<Value>)arg1).Append((ListValue<Value>)arg2)); else throw new Error(BadArgs());
                else if (name == "=") return new BoolValue(arg1.EqualValue(arg2, style));
                else if (name == "<>") return new BoolValue(!arg1.EqualValue(arg2, style));
                ...
                else if (name == "arctan2") if (arg1 is NumberValue arg1As && arg2 is NumberValue arg2As) return new NumberValue(Math.Atan2(arg1As.value, arg2As.value)); else throw new Error(BadArgs());
                else if (name == "max") if (arg1 is NumberValue arg1As && arg2 is NumberValue arg2As) return new NumberValue(Math.Max(arg1As.value, arg2As.value)); else throw new Error(BadArgs());
                else if (name == "min") if (arg1 is NumberValue arg1As && arg2 is NumberValue arg2As) return new NumberValue(Math.Min(arg1As.value, arg2As.value)); else throw new Error(BadArgs());
