...
namespace KaemikaXM {
    ...
    public class SKDevicePainter : DevicePainter {
        private SKCanvas canvas;

        public SKDevicePainter(SKCanvas canvas) {
            this.canvas = canvas;
        }

        public override void Draw(int canvasX, int canvasY, int canvasWidth, int canvasHeigth) { 

            (float margin, float padRadius) = device.FittedDimensions(canvasWidth, canvasHeigth);
            float strokeWidth = padRadius / 10.0f;
            float accentStrokeWidth = 1.5f * strokeWidth;
            float textStrokeWidth = accentStrokeWidth / 2.0f;

            if (device.deviceImage == null || device.deviceImage.Width != canvasWidth || device.deviceImage.Height != canvasHeigth) {
                device.deviceImage = new SKBitmap(canvasWidth, canvasHeigth);
                using (var backgroundCanvas = new SKCanvas(device.deviceImage)) {
                    DrawDevice(device, backgroundCanvas, canvasX, canvasY, canvasWidth, canvasHeigth, padRadius, margin, device.pinchPan);
                }
            }

            if (device.deviceImage != null) canvas.DrawBitmap(device.deviceImage, 0, 0); // do not apply pinchPan: background bitmap is alread scaled by it
